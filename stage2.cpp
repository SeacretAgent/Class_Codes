...
void Compiler::createListingHeader()
{
	time_t now = time (NULL);
	 ...
	 listingFile << "STAGE2:  Connor Flenniken, Devoux Deysel\t" << ctime(&now) << "\n";
...
{ 
	 ...
	 nextToken();
	 if (numBegins > 1 && token.compare(";") != 0)
		processError("semicolon expected\n");
	 else if (numBegins == 1 && token.compare(".") != 0)
...
{
	if (token != "end" && token != "begin" && token != "until")
	{
		execStmt();
		execStmts();
	}
...
void Compiler::factors()
{
	if (token == "*" || token == "div" || token == "mod" || token == "and")
	{
		...
		string operand1 = popOperand();
		string operand2 = popOperand();
		//objectFile << operand1 << " " << operand2 << endl;
		code(popOperator(), operand1, operand2);
		if (token == "*" || token == "div" || token == "mod" || token == "and")
			factors();
		if (isNonKeyId(token) && !isKeyword(token))
...
void Compiler::part()
{
	...
	if (isInteger(token))
	{
		...
		if (symbolTable.find(token) == symbolTable.end())
		{
			...
			if (found && (itr->first != "false" && itr->first != "true"))
			{
				token = itr->first;
			}
			else
...
void Compiler::emitAssignCode(string operand1, string operand2)
{
	...
	{
		listingFile << operand1 << " " << operand2 << endl;
...
void Compiler::emitThenCode(string operand1, string)
{
	...
	//the type of operand1 is not boolean
	if (whichType(operand1) != BOOLEAN)
		processError("\"if\" predicate must be of type boolean\n");
	tempLabel = getLabel();
	
	//operand1 is not in the A register then
	if (contentsOfAReg != operand1)
	{
...
string Compiler::nextToken() //returns the next token or end of file marker
{
	 ...
	 while (token.empty())
	 {
		...
		else if ((islower(ch)) && tokenCounter < 15)
		{
			token += ch;
			tokenCounter++;
